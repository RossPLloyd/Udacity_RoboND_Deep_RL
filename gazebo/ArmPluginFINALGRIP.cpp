 ...
 * Completed classwork by Ross Lloyd, Udacity Robotics Nanodegree Term2, Deep RL Manipulator Project
 */
...
#define GAMMA 0.9f
#define EPS_START 0.9f //0.9
#define EPS_END 0.02f //0.05
...
#define COLLISION_GMID   "arm::grippermiddle::middle_collision"
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	{
		//std::cout << "i: " << i << "Contacts size: " << contacts->contact_size() << std::endl;
		if( strcmp(contacts->contact(i).collision2().c_str(), COLLISION_FILTER) == 0 )
			continue;
		//{std::cout << "contents of string: " << contacts->contact(i).collision2().c_str() << std::endl;}
		...
		// if the detected collision with tube is NOT the floor and	the detected collision IS the gripper base....
		if (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_FILTER) != 0 && strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0)
		{
			//announce the collision object
			std::cout << "collision is: " << contacts->contact(i).collision2().c_str() << std::endl;
			// assign a positive reward and end episode
		...
		// if the detected collision with tube is NOT the floor and	the detected collision IS link 2....
		else if (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_FILTER) != 0 && strcmp(contacts->contact(i).collision2().c_str(), COLLISION_LINK2) == 0)
		{
			std::cout << "collision is: " << contacts->contact(i).collision2().c_str() << std::endl;
			rewardHistory = REWARD_LOSS;
			// assign a negative reward and end episode
			newReward  = true;
			endEpisode = true;

			return;
		}
		// if the detected collision with tube is NOT the floor and	the detected collision IS the middle finger link of the gripper....
		else if (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_FILTER) != 0 && strcmp(contacts->contact(i).collision2().c_str(), COLLISION_GMID) == 0)
		{
			std::cout << "collision is: " << contacts->contact(i).collision2().c_str() << std::endl;
			rewardHistory = REWARD_LOSS;
			newReward  = true;
...
bool ArmPlugin::updateAgent()
{
	...
	//Use conditional operator to assess modulo
	float velocity = ref[jointIndex] + actionVelDelta * ((action % 2 == 0) ? 1.0f : -1.0f);	
	...
	//// TODO - Set joint position based on whether action is even or odd.
        float joint = ref[jointIndex] + actionJointDelta * ((action % 2 == 0) ? 1.0f : -1.0f);
	...
	if( hadNewState && !newReward )
	{
		...
		if(!(gripBBox.min.z <= groundContact))
		{
			...
			{			
				...
				// subtract a small penalty from the multiplied avgGoalDelta to prevent arm oversailing the object
				rewardHistory = (avgGoalDelta * REWARD_BOOST) -0.35f;

				
			newReward     = true;	
			}
			//store the last goal distance
